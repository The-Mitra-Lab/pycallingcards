...
class AlignmentTagger(BarcodeParser):
	...
	def tag_read(self, read, decompose_barcode: bool = True):
		...
		Args:
			...
			decompose_barcode (bool): if the barcode is appended as a read identifer
			on the bam id line, rather than an already decomposed tag string,
			then extract the barcode and evaluate it against expectations in
			the barcode_details json. Default to True.
		...
		if decompose_barcode:
			...
			for k, v in barcode_dict['details'].items():
				...
				if bam_tag:
					tag_dict[bam_tag] = v.get('name')+'/'+str(v.get('dist'))
		else:
