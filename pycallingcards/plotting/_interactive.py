...
def interactive(
    ...
    for i in range(len(data)):
        ...
        if data[i]["mode"] == "ccf":
            ...
            xvalue = np.array(data[i]["data"][ (data[i]["data"]["Chr"] == chrom ) &  (data[i]["data"]["Start"] >= start) &  (data[i]["data"]["End"] <= end) ]["Start"])
            
            yvalue = np.log(list(data[i]["data"][ (data[i]["data"]["Chr"] == chrom ) &  (data[i]["data"]["Start"] >= start) &  (data[i]["data"]["End"] <= end)  ]["Reads"] +1)) - add
            ...
            Start = list(data[i]["data"][(data[i]["data"]["Chr"] == chrom ) &  (data[i]["data"]["Start"] >= start) &  (data[i]["data"]["End"] <= end)  ]['Start'])
            End = list(data[i]["data"][(data[i]["data"]["Chr"] == chrom ) &  (data[i]["data"]["Start"] >= start) &  (data[i]["data"]["End"] <= end)  ]['End'])
