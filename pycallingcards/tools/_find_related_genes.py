...
def pair_peak_gene_sc(
    ...
    peak_annotation["name"] = peak_annotation["Chr"] +"_"+ peak_annotation["Start"].astype(str) +"_"+ peak_annotation["End"].astype(str)
    ...
    for cluster in range(pvalue_ccf.shape[1]):
        for peak in range(pvalue_ccf.shape[0]):
            if pvalue_ccf[peak,cluster] <= pvalue_cutoff_ccf and abs(lfg_ccf[peak,cluster]) >= lfc_cutoff:
                rnalist = list(peak_annotation[peak_annotation["name"] == name_ccf[peak,cluster]][["Gene Name1","Gene Name2"]].iloc[0])
                for rna in list(set((rnalist))):
                    if rna in rnalists:
                        if  pvalue_adata[:,cluster][name_adata[:,cluster] == rna] <= pvalue_cutoff_rna and abs(score_adata[:,cluster][name_adata[:,cluster] == rna]) >= score_cutoff:
                            
                            result.append([possible_group[cluster],
                            name_ccf[peak,cluster],lfg_ccf[peak,cluster],pvalue_ccf[peak,cluster],
                            rna,score_adata[:,cluster][name_adata[:,cluster] == rna][0],pvalue_adata[:,cluster][name_adata[:,cluster]== rna][0]])
            else:
                continue
            
                
    return pd.DataFrame(result, columns= ["Cluster", "Peak", "Logfoldchanges","Pvalue_peak",  "Gene", "Score_gene", "Pvalue_gene"])
...
def pair_peak_gene_bulk(
    ...
    for peak in range(len(peaks)):
        if (de_pd.iloc[peak,1] <= pvalue_cutoff_ccf) and (abs(de_pd.iloc[peak,2]) >= lfc_cutoff_ccf):

            genes = list(peak_rna.iloc[peak])
            for gene in genes:
                if (gene in rna_list) and (rnade.loc[gene]['pvalue'] <= pvalue_cutoff_rna) and (abs(rnade.loc[gene]['log2FoldChange']) >= lfc_cutoff_rna) :     
                    result.append([peaks[peak],de_pd.iloc[peak,2],de_pd.iloc[peak,1],
                                    gene, rnade.loc[gene]['log2FoldChange'], rnade.loc[gene]['pvalue']])
        
    return pd.DataFrame(result,columns = ['Peak',name_ccf +'_peak','Pvalue_peak','Gene','Score_gene','Pvalue_peak'])